...
export default class Cart {
  ...
  renderModal() {
    ...
    for (let { product, count } of this.cartItems) {
      this.setBody.append(this.renderProduct(product, count));
    }

    this.setBody.append(this.renderOrderForm());
    this.setBody.addEventListener("click", this.onModalBodyClick);
    this.setBody.querySelector("form").onsubmit = (event) => this.onSubmit(event);
    this.modal.setBody(this.setBody);
  ...
  onProductUpdate({product, count}) {
    ...
    if (count == 0) {
      this.setBody.querySelector(`[data-product-id="${product.id}"]`).remove();
    } else {
      this.setBody.querySelector(`[data-product-id="${product.id}"] .cart-counter__count`)
      .innerHTML = count;

      this.setBody.querySelector(`[data-product-id="${product.id}"] .cart-product__price`)
      .innerHTML = '€' + (count * product.price).toFixed(2);
    }


    this.setBody.querySelector(`.cart-buttons__info-price`)
  ...
  async onSubmit(event) {
    ...
    let form = this.setBody.querySelector('.cart-form');
